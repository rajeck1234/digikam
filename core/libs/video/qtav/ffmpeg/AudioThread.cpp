...
AudioThread::AudioThread(QObject* const parent)
...
*/
    if (!d.filters.isEmpty())
    {
        ...
        {
            AudioFilter* const af = static_cast<AudioFilter*>(filter);
    ...
    while (!d.stop)
    {
        ...
        if (d.seek_requested)
        {
            ...
            qCDebug(DIGIKAM_QTAV_LOG).noquote()
                << QString::asprintf("request seek audio thread");
        ...
        else
        {
            ...
            {
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("audio thread wait to sync end for sync id: %d",
                        d.clock->syncId());
...
/*
            qCDebug(DIGIKAM_QTAV_LOG).noquote()
                << QString::asprintf("eof pkt: %d valid: %d, aqueue size: %d, abuffer: %d %.3f %d, fake_duration: %lld",
                    pkt.isEOF(), pkt.isValid(), d.packets.size(), d.packets.bufferValue(),
                    d.packets.bufferMax(), d.packets.isFull(), fake_duration);
*/
            ...
            if (pkt.isEOF())
            {
                ...
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("audio thread gets an eof packet. pkt.pts: %.3f, d.render_pts0:%.3f",
            ...
            if (!pkt.isValid())
            {
                if (pkt.pts >= 0)
                {
                    ...
                    qCDebug(DIGIKAM_QTAV_LOG).noquote()
                        << QString::asprintf("Invalid packet! flush audio codec context!!!!!!!! audio queue size=%d",
                            d.packets.size());
                    ...
                    qCDebug(DIGIKAM_QTAV_LOG).noquote()
                        << QString::asprintf("audio seek: %.3f, id: %d",
...
/*
                        qCDebug(DIGIKAM_QTAV_LOG).noquote()
                            << QString::asprintf("fake_duration update on seek: %ul + %ul - %.3f",
                                fake_duration, fake_pts, d.render_pts0);
*/
...
/*
                    qCDebug(DIGIKAM_QTAV_LOG).noquote()
                        << QString::asprintf("get fake apkt: %.3f+%ul",
                            pkt.pts, fake_duration);
*/
                    continue;
                }
            }
...
/*
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("fake_wait: %ul, fake_duration: %lld, delay: %.3f",
                        ms, fake_duration, d.clock->delay());
*/
        ...
        // if audio stream is too short, seeking will fail and d.render_pts0 keeps >0

        bool skip_render = ((pkt.pts >= 0) && (pkt.pts < d.render_pts0));
        ...
        if (skip_render)
        {
            ...
            qCDebug(DIGIKAM_QTAV_LOG).noquote()
                << QString::asprintf("skip audio decode at %f/%f v=%f a-v=%fms",
                    dts, d.render_pts0, d.clock->videoTime(), a_v * 1000.0);
        ...
        if (is_external_clock && !pkt.isEOF())
        {
            ...
            if (qAbs(d.delay) < 2.0)
            {
                ...
                {
                    // Speed up. drop frame? resample?

                    qCDebug(DIGIKAM_QTAV_LOG).noquote()
                        << QString::asprintf("audio is late compared with external clock. skip decoding. %.3f-%.3f=%.3f",
                            dts, d.clock->value(), d.delay);
            ...
            else
            {
                ...
                else
                {
                    ...
                    qCDebug(DIGIKAM_QTAV_LOG).noquote()
                        << QString::asprintf("audio is too late compared with external clock. skip decoding. %.3f-%.3f=%.3f",
                            dts, d.clock->value(), d.delay);
        ...
        QMutexLocker locker(&d.mutex);
        Q_UNUSED(locker);
        AudioDecoder* const dec = static_cast<AudioDecoder*>(d.dec);
        ...
        if (has_ao && dec->resampler())
        {
                ...
                (dec->resampler()->outAudioFormat() != ao->audioFormat()))
            {
                ...
                if (d.resample)
                {
                    ...
                    qCDebug(DIGIKAM_QTAV_LOG).noquote()
                        << QString::asprintf("decoder set speed: %.2f",
                            ao->speed());
        ...
        {
            qCDebug(DIGIKAM_QTAV_LOG).noquote()
                << QString::asprintf("audio thread stop before decode()");

            break;
        }
/*
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("apkt: %.3f, %lld %p",
                pkt.pts, pkt.asAVPacket()->pts, pkt.asAVPacket()->data);
*/
        if (!dec->decode(pkt))
        {
            qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote()
                << QString::asprintf("Decode audio failed. undecoded: %d",
                    dec->undecodedSize());

            if (pkt.isEOF())
            {
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("audio decode eof done");
                ...
                {
                    qCDebug(DIGIKAM_QTAV_LOG).noquote()
                        << QString::asprintf("audio seek done at eof pts: %.3f. id: %d",
                            pkt.pts, sync_id);
        ...
        if (d.render_pts0 >= 0.0)
        {
            ...
            {
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("skip audio rendering: %f-%f",
                        frame.timestamp(), d.render_pts0);

                continue; // pkt data is updated after decode, no reset here
            }

            qCDebug(DIGIKAM_QTAV_LOG).noquote()
                << QString::asprintf("audio seek finished @%.3f. id: %d",
                    frame.timestamp(), sync_id);
...
/*
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("frame samples: %d @%.3f+%lld",
                frame.samplesPerChannel()*frame.channelCount(), frame.timestamp(), frame.duration() / 1000LL);
*/
        while (decodedSize > 0)
        {
            if (d.stop)
            {
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("audio thread stop after decode()");

                break;
            }

            const int chunk = qMin(decodedSize, has_ao ? ao->bufferSize()
                                                       : 512 * frame.format().bytesPerFrame()); // int(max_len * byte_rate));
...
/*
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("ao.timestamp: %.3f, pts: %.3f, pktpts: %.3f",
                        ao->timestamp(), pts, pkt.pts);
*/
    ...
    qCDebug(DIGIKAM_QTAV_LOG).noquote()
        << QString::asprintf("Audio thread stops running...");
}
