...
const char* VideoShader::vertexShader() const
{
    ...
    if (vert.isEmpty())
    {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote()
            << QString::asprintf("Empty vertex shader!");
...
const char* VideoShader::fragmentShader() const
{
    ...
    if (d.video_format.isPlanar())
    {
        ...
        if (bpc > 8)
        {
            // has and use 16 bit texture (r16 for example):
            // If channel depth is 16 bit, no range convertion required.
            // Otherwise, must convert to color.r*(2^16-1)/(2^bpc-1)
...
void VideoShader::initialize(QOpenGLShaderProgram* shaderProgram)
{
    ...
    qCDebug(DIGIKAM_QTAV_LOG).noquote()
        << QString::asprintf("uniform locations:");
    ...
    qCDebug(DIGIKAM_QTAV_LOG).noquote()
        << QString::asprintf("u_Matrix: %d",      d.u_Matrix);

    qCDebug(DIGIKAM_QTAV_LOG).noquote()
        << QString::asprintf("u_colorMatrix: %d", d.u_colorMatrix);

    qCDebug(DIGIKAM_QTAV_LOG).noquote()
        << QString::asprintf("u_opacity: %d",     d.u_opacity);

    if (d.u_c >= 0)
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("u_c: %d", d.u_c);
    }

    if (d.u_to8 >= 0)
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("u_to8: %d", d.u_to8);
    }

    if (d.u_texelSize >= 0)
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("u_texelSize: %d", d.u_texelSize);
    }

    if (d.u_textureSize >= 0)
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("u_textureSize: %d", d.u_textureSize);
    }
    ...
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("user uniform locations in vertex shader:");
    ...
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("user uniform locations in fragment shader:");
...
bool VideoShader::update(VideoMaterial* const material)
{
    ...
    if ((mt != d.material_type) || d.rebuild_program)
    {
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("Rebuild shader program requested: %d. Material type %d=>%d",
                d.rebuild_program, d.material_type, mt);
...
QByteArray VideoShader::shaderSourceFromFile(const QString& fileName) const
{
    ...
    {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote()
            << QString::asprintf("Can not load shader %s: %s",
                f.fileName().toUtf8().constData(), f.errorString().toUtf8().constData());
...
bool VideoShader::build(QOpenGLShaderProgram* const shaderProgram)
{
    if (shaderProgram->isLinked())
    {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote()
...
void VideoMaterial::setCurrentFrame(const VideoFrame& frame)
{
    ...
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("texture target: %#x=>%#x",
                d.target, new_target);
...
{
    VideoShader* const shader = new VideoShader();
...
{
    return QString::fromUtf8("gl material 16to8bit: %1, planar: %2, has alpha: %3, "
                             "2d texture: %4, 2nd plane rg: %5, xyz: %6")
            .arg(!!(value & 1))
            .arg(!!(value & (1 << 1)))
            .arg(!!(value & (1 << 2)))
            .arg(!!(value & (1 << 3)))
...
qint32 VideoMaterial::type() const
{
    ...
    const int rg_biplane    = (
                               (fmt.planeCount() == 2)               &&
                               !OpenGLHelper::useDeprecatedFormats() &&
                               OpenGLHelper::hasRG()
                              );
    ...
    const int channel16_to8 = (
                               (d.bpc > 8) &&
                               (
                                (OpenGLHelper::depth16BitTexture() < 16) ||
                                !OpenGLHelper::has16BitTexture()         ||
                                fmt.isBigEndian()
                               )
                              );
...
void VideoMaterialPrivate::uploadPlane(int p, bool updateTexture)
{
    ...
    if (!frame.constBits(0))
    {
        ...
        if (frame.map(GLTextureSurface, &tex, p))
        {
            ...
            // glActiveTexture was called, but maybe bind to 0 in map
...
/*
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("bind PBO %d", p);
*/
        QOpenGLBuffer& pb = pbo[p];
        pb.bind();

        // glMapBuffer() causes sync issue.
        // Call glBufferData() with nullptr pointer before glMapBuffer(),
        // the previous data in PBO will be discarded and
        // glMapBuffer() returns a new allocated pointer or
        // an unused block immediately even if GPU is still working with the previous data.
        // https://www.opengl.org/wiki/Buffer_Object_Streaming#Buffer_re-specification

        pb.allocate(pb.size());
        GLubyte* const ptr = (GLubyte*)pb.map(QOpenGLBuffer::WriteOnly);
...
QSizeF VideoMaterial::texelSize(int plane) const
{
    ...
    return QSizeF(1.0 / (qreal)d.texture_size[plane].width(),
                  1.0 / (qreal)d.texture_size[plane].height());
}
...
QPointF VideoMaterial::mapToTexture(int plane, const QPointF& p, int normalize) const
{
    ...
    // only apply to unnormalized input roi

    const qreal s     = tex0W / qreal(d.width);
                                       ...
                                       GLenum dataType, int width, int height)
{
    ...
    DYGL(glTexParameteri(target, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
    DYGL(glTexParameteri(target, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
    DYGL(glTexImage2D(target, 0, internal_format, width, height, 0 /*border, ES not support*/,
...
bool VideoMaterialPrivate::updateTextureParameters(const VideoFormat& fmt)
{
    ...
    for (int i = 0 ; i < nb_planes ; ++i)
    {
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("texture width: %d - %d = pad: %d. bpp(gl): %d",
                texture_size[i].width(), effective_tex_width[i], pad, bpp_gl);

        if (target == GL_TEXTURE_RECTANGLE)
            v_texel_size[i] = QVector2D(1.0, 1.0);
        else
            v_texel_size[i] = QVector2D(1.0 / (float)texture_size[i].width(),
                                        1.0 / (float)texture_size[i].height());
    }
    ...
    if (textures.size() > nb_planes)
    {
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("try to delete %d textures", nb_delete);
        ...
        if (!textures.isEmpty())
        {
            for (int i = 0 ; i < nb_delete ; ++i)
            {
                ...
                qCDebug(DIGIKAM_QTAV_LOG).noquote()
                    << QString::asprintf("try to delete texture[%d]: %u. can delete: %d",
                        nb_planes+i, t, owns_texture[t]);
...
bool VideoMaterialPrivate::ensureResources()
{
    ...
    if (update_textures                         ||
        ...
        (linsize0 != plane0Size.width())        ||
        (frame.height() != plane0Size.height()) ||
        ((plane1_linesize > 0) && (frame.bytesPerLine(1) != plane1_linesize)))
    {
        ...
        {
            qCDebug(DIGIKAM_QTAV_LOG).noquote()
                << QString::asprintf("plane linesize %d: padded = %d, effective = %d. theoretical plane size: %dx%d",
                    i, frame.bytesPerLine(i), frame.effectiveBytesPerLine(i), 
                    frame.planeWidth(i), frame.planeHeight(i));

            // we have to consider size of opengl format.
            // set bytesPerLine here and change to width later

            texture_size[i]           = QSize(frame.bytesPerLine(i), frame.planeHeight(i));

            // store bytes here, modify as width later

            effective_tex_width[i]    = frame.effectiveBytesPerLine(i); 

            // usually they are the same. If not, the difference is small.
            // min value can avoid rendering the invalid data.

            effective_tex_width_ratio = qMin(effective_tex_width_ratio,
                                             (qreal)frame.effectiveBytesPerLine(i) / (qreal)frame.bytesPerLine(i));
        }
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote()
            << QString::asprintf("effective_tex_width_ratio=%f",
                effective_tex_width_ratio);
...
bool VideoMaterialPrivate::ensureTextures()
{
    ...
    for (int p = 0 ; p < nb_planes ; ++p)
    {
        ...
        if (tex)
        {
            // can be 0 if resized to a larger size

            qCDebug(DIGIKAM_QTAV_LOG).noquote()
                << QString::asprintf("try to delete texture for plane %d (id=%u). can delete: %d",
                    p, tex, owns_texture[tex]);
